...
5. If the artifact is a workstation image or personal environment setup, place it under `workstation/`
   - iximiuz playground images (rootfs builds) also go under `workstation/` since they produce
     an interactive developer environment, not a production service
6. If the task spans two domains, place it where someone would first look for it
7. Never place new content entries inside `docs/` - that folder is not for runbook entries
